...
struct ApiRequestNP{
    func requestApiNP(page: Int = 1, completionHandler: @escaping ([Movie]) -> Void){
        if page < 0 { fatalError("Page should not be lower than 0") }
        let key = "06ba203a54577bd8c91c3b4d4b14d0ec"
        let urlString = "https://api.themoviedb.org/3/movie/now_playing?api_key=\(key)&sort_by=popularity.desc&page=\(page)"
